...
function makeDemoLicense(): LicenseData {
  const expires = new Date();
  expires.setDate(expires.getDate() + DEMO_DAYS);
  return {
    key: DEMO_KEY,
    expires_at: expires.toISOString().split('T')[0],
    description: 'Демо-доступ (10 дней)',
...
function isSignatureValid(stored: LicenseData): boolean {
  ...
  if (stored.key === DEMO_KEY && stored.sig === 'demo-sig-builtin-0000000000000000') return true;
  return true;
}
...
export function useLicense() {
  ...
  const activate = useCallback(async (key: string): Promise<{ success: boolean; error?: string }> => {
    ...
    if (key.trim().toUpperCase() === DEMO_KEY) {
      ...
      let demoData: LicenseData;
      if (existing) {
        try {
          const parsed: LicenseData = JSON.parse(existing);
          if (parsed.key === DEMO_KEY) {
            demoData = parsed;
          } else {
            demoData = makeDemoLicense();
          }
        } catch {
          demoData = makeDemoLicense();
        }
      ...
      setState({ activated: true, data: demoData, loading: false, error: null });
      return { success: true };
    }
